...
- User asks about the **status of a past purchase / claim / EMI / repair** — "what's the status of X?", "where is my claim?", "any EMI running?", "what did I pay for Y?" — answer **from the record, don't re-research** (see Tracking & Recall)
...
14. **Save where the user wants.** Ask before recording; by default output the record in chat. Only write a file (tracker note, the user's CSV, their own file) or offer a download when asked — never into the skill's own files. Once saved, the record answers later status questions — **recall, don't re-research** (see Tracking & Recall).
...
Half of tracking is recording; the other half is **reading the record back later**. When the user asks *"what's the status of X?"*, *"where is my claim?"*, *"any EMI running?"*, *"what did I pay for Y?"* — that is a **recall job**: answer **from the record, don't re-research the product**. Full playbook: `references/recall.md`.

1. **Source, in order:** the user's vault tracker/note (local) → the AI platform's memory (same project/thread on web) → the user's attached `my-deals.csv` (any fresh chat).
2. **Answer always shows:** product, date, effective price, **current status**, **where it's recorded** (file/row/note), and **next action** (e.g. "claim open — if the brand ignores >7 days, NCH 1915"; "EMI active, 4 of 12 paid, due date the 5th").
3. **Canonical status words** (one word, one meaning): deal `bought`/`buying`/`waiting`/`cancelled`; claim `open`/`approved`/`rejected`/`refund-issued`/`escalated`; EMI `active`/`closed`/`early-closed`; repair `done`/`warranty`/`oow`. Normalize the record's words to these.
4. **Export any format:** on request, give the whole record as `my-deals.csv`, a markdown table, or a note — whatever the user wants.
5. **Not tracked yet?** Say "not found — record it now?" and emit the row to add. Never make the user dig through their own notes.
...
- `references/recall.md` — the Record & Recall playbook: record one row per purchase (web agents output a copy-paste row), recall answers from the record (source order, answer format, canonical status vocabulary, export-any-format). Load for any "status of X / where is my claim / any EMI running?" question, and for recording in web/cloud agents.
...
- `assets/my-deals.csv` — merged blank CSV **template** (deal + claim + EMI + repair on one row per purchase) — the portable everything-in-one record for users who keep a single file; read-only, output in chat by default.
